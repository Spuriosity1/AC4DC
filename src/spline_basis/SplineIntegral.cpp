...
// just aliases (type-safe #defines)
static const double* gaussX_EII =  gauss_quad::X_10;
static const double* gaussW_EII =  gauss_quad::W_10;
static const double* gaussX_TBR =  gauss_quad::X_10;
static const double* gaussW_TBR =  gauss_quad::W_10;
static const double* gaussX_EE = gauss_quad::X_4;
static const double* gaussW_EE = gauss_quad::W_4;


static const double* gaussX_sqrt = gauss_sqrt_quad::X_10;
static const double* gaussW_sqrt = gauss_sqrt_quad::W_10;
    ...
    for (size_t eta = 0; eta < eii.fin.size(); eta++) {
        ...
        for (auto&& boundary : this->knots_between(a, b)) {
            ...
            for (int i=0; i<GAUSS_ORDER_EII; i++) {
                ...
                tmp2 += gaussW_EII[i]* (*this)(K, e)*pow(e,0.5)*ImpactCrossSection::sigmaBEB(e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
            }
        ...
        // tmp *= pow(e,0.5) * ImpactCrossSection::sigmaBEB(e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
        // tmp *= 1.4142135624;
    ...
    for (size_t eta = 0; eta < eii.fin.size(); eta++) {
        ...
        if (b > a) {
            for (int i=0; i<GAUSS_ORDER_EII; i++) {
                ...
                tmp += gaussW_EII[i]* (*this)(K, e)*pow(e,0.5)*ImpactCrossSection::sigmaBEB(e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
            }
            tmp *= (b-a)/2;
        ...
        for (int i=0; i<GAUSS_ORDER_TBR; i++) {
            ...
            for (int j=0; j<GAUSS_ORDER_TBR; j++) {
                ...
                tmp2 += gaussW_TBR[j]*(*this)(L, s)*ep*pow(s,-0.5)*
                    ImpactCrossSection::DsigmaBEB(ep, e, B, tbr.kin[eta], tbr.occ[eta]);
            }
            tmp2 *= (bL-aL)/2;
    ...
    for (size_t eta = 0; eta<eii.fin.size(); eta++)
    {
            ...
            for (int j=0; j<GAUSS_ORDER_EII; j++)
            {
                ...
                for (int k=0; k<GAUSS_ORDER_EII; k++) {
                    ...
                    tmp2 += gaussW_EII[k]*(*this)(K, ep)*pow(ep,0.5)*
                        ImpactCrossSection::DsigmaBEB(ep, e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
                }
        ...
        for (auto&& boundary : this->knots_between(min_JK, max_JK)) {
            ...
            for (int k=0; k<GAUSS_ORDER_EII; k++)
            {
                ...
                tmp += gaussW_EII[k]*pow(e, 0.5)*raw_bspline(J, e)*(*this)(K, e)*ImpactCrossSection::sigmaBEB(e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
            }
            retval -= tmp*diff;
        }
...
double SplineIntegral::calc_Q_eii( const RateData::EIIdata& eii, size_t J, size_t K) const {
    ...
    for (size_t eta = 0; eta<eii.fin.size(); eta++)
    {
        ...
        for (int k=0; k<GAUSS_ORDER_EII; k++)
        {
            ...
            // tmp += gaussW_EII[k]*pow(e, 0.5)*(*this)(J, e)*(*this)(K, e)*ImpactCrossSection::sigmaBEB(e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
            tmp += gaussW_EII[k]*pow(e, 0.5)*raw_bspline(J, e)*(*this)(K, e)*ImpactCrossSection::sigmaBEB(e, eii.ionB[eta], eii.kin[eta], eii.occ[eta]);
        }
        tmp *= (max_JK-min_JK)/2;
        retval -= tmp;
            ...
            for (size_t xi=0; xi<tbr.fin.size(); xi++) {
                ...
                for (int j=0; j<GAUSS_ORDER_TBR; j++) {
                    ...
                    for (int k=0; k<GAUSS_ORDER_TBR; k++) {
                        ...
                        double e = s + ep + B;
                        // tmp2 += gaussW_TBR[k] * ( e ) * pow(s*ep,-0.5) * ImpactCrossSection::DsigmaBEB(e, ep, B, tbr.kin[xi], tbr.occ[xi])*(*this)(J, e)*(*this)(L, s);
                        tmp2 += gaussW_TBR[k] * ( e ) * pow(s*ep,-0.5) * ImpactCrossSection::DsigmaBEB(e, ep, B, tbr.kin[xi], tbr.occ[xi])*raw_bspline(J, e)*(*this)(L, s);
                    }
                ...
                for (size_t xi=0; xi<tbr.fin.size(); xi++) {
                    ...
                    for (int j=0; j<GAUSS_ORDER_TBR; j++) {
                        ...
                        for (int k=0; k<GAUSS_ORDER_TBR; k++) {
                            ...
                            tmp2 += gaussW_TBR[k]*(s+e+B)*pow(e * s,-0.5)*(*this)(L,s)*ImpactCrossSection::DsigmaBEB(s+e+B, e, B, tbr.kin[xi], tbr.occ[xi]);
                        }

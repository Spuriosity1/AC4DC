...
//	Interpolate orbitals on the new grid
	...
	for (int i = 0; i < Orbitals.size(); i++) {
		if (input.orbital_potential != HFInput::e_pot_t::V_Nm1 && orbitals[i].occupancy() == 0) continue;
...
// main loop over all the possible transitions
	for (int i = 0; i < Orbitals.size(); i++)
	{
		if (Orbitals[i].occupancy() > 0)
		{
			...
			PhotoTmp.hole = i;
			PhotoTmp.val = 0;
			if (input.orbital_potential == HFInput::e_pot_t::V_Nm1no) Orbitals[i].set_occupancy(j - 1);//excite one electron...
			Continuum.Energy = omega + Orbitals[i].Energy;//... into continuum
			if (Continuum.Energy > 0)
			{
				// Update here if Input.Hamiltonian == "LDA".
				if (input.ham_model == HFInput::ham_mod_t::LDA) {
				...
				if (input.orbital_potential == HFInput::e_pot_t::V_Nm1) U.HF_V_N1(&Continuum, Orbitals, i, true, false);
			...
			if (input.orbital_potential == HFInput::e_pot_t::V_Nm1no) Orbitals[i].set_occupancy(j);
...
std::vector<auger> DecayRates::Auger(std::vector<int> Max_occ, std::ofstream & log)
{
	...
	for (int i = 0; i < orbitals.size(); i++) {
		...
		if (input.orbital_potential == HFInput::e_pot_t::V_Nm1 || i <= allowed) W.RecalcWF(orbitals[i], lattice, Orbitals[i], Lattice);
		Infinity = lattice.R(orbitals[i].pract_infinity());
		int j = 0;
		while (Lattice.R(j) < Infinity && j < Lattice.size() - 1) j++;
		Orbitals[i].set_infinity(j);
	...
	// convenience
	const bool is_hollowshell = (input.orbital_potential == HFInput::e_pot_t::V_Nm1no);
	...
	for (int h = 0; h < allowed; h++)
	{
		if (Orbitals[h].occupancy() < Max_occ[h])//check if there is a hole
		{
			...
			if (is_hollowshell) Orbitals[h].set_occupancy(h_occ + 1);
			for (int f = h + 1; f <= allowed; f++)//fill the hole
			{
				Tmp.fill = f;
				f_occ = Orbitals[f].occupancy();
				if (f_occ == 0) continue;
				if (is_hollowshell) Orbitals[f].set_occupancy(f_occ - 1);
				for (int e = f; e <= allowed; e++)//eject
				{
					e_occ = Orbitals[e].occupancy();
					if (e_occ == 0) continue;
					if (e == f &&  is_hollowshell && e_occ < 1) continue;
					if (e == f && !is_hollowshell && e_occ < 2) continue;
					if (e == f && !is_hollowshell ) e_occ--;
					double T = 0;
					Tmp.eject = e;
					Tmp.val = 0;
					...
					if (input.orbital_potential == HFInput::e_pot_t::V_Nm1no) Orbitals[e].set_occupancy(e_occ - 1);
					Tmp.energy = E_cont;
					...
					if (is_hollowshell) Orbitals[e].set_occupancy(e_occ);
				}
				if (is_hollowshell) Orbitals[f].set_occupancy(f_occ);
			}
			if (is_hollowshell) Orbitals[h].set_occupancy(h_occ);
		}
	}
...
int DecayRates::IntegrateContinuum(Grid &Lattice, Potential &U, std::vector<RadialWF> &Core, RadialWF* Current, int c)
{
	...
	while (fabs(new_Phase / Phase - 1) > accuracy || fabs(new_Norm / Norm - 1) > accuracy || m < 2)
	{
		...
		if (input.ham_model == HFInput::ham_mod_t::HF) U.HF_upd_exc(Current, Core);
		if (input.orbital_potential == HFInput::e_pot_t::V_Nm1) U.HF_V_N1(Current, Core, c, false, true);

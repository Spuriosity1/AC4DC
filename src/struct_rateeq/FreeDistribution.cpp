...
void Distribution::add_maxwellian(double T, double N) {
    ...
    for (size_t i=0; i<size; i++) {
        ...
        for (size_t j=0; j<10; j++) {
            double e = (b-a)/2 *gauss_quad::X_10[j] + (a+b)/2;
            v[i] +=  gauss_quad::W_10[j]*exp(-e/T)*basis(i, e)*pow(e,0.5);;
        }
        v[i] *= (b-a)/2;
...
double Distribution::integral(double (g)(double)) {
    ...
    for (size_t J = 0; J < Distribution::size; J++)
    {
        ...
        {
            double e = gauss_quad::X_10[i]*(b-a)/2 + (b+a)/2;
            tmp += gauss_quad::W_10[i]*basis(J, e)*g(e);
        }

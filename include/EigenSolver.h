...
public:
	...
	void SolveSystem(std::vector<std::vector<double>> &A, std::vector<double> &B, int Length);
	...
	void SolveGenEig(std::vector<std::vector<double>> &F, std::vector<std::vector<double>> &S);
	// Same as above, but S is diagonal. It's elements are conmonents of input vector S.
	void SolveGenEig(std::vector<std::vector<double>> &F, std::vector<double> &S);
